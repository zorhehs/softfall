...
enum ParticleTextures {
    ...
    static func streak(width: Int, height: Int, peakAlpha: CGFloat = 0.9) -> CGImage? {
        ...
        let colors = [
            CGColor(colorSpace: space, components: [0.87, 0.92, 1.0, peakAlpha])!,
            CGColor(colorSpace: space, components: [0.80, 0.88, 1.0, peakAlpha * 0.35])!,
            CGColor(colorSpace: space, components: [0.75, 0.85, 1.0, 0])!
        ]
        guard let gradient = CGGradient(
            colorsSpace: space,
            colors: colors as CFArray,
    ...
    static var raindrop: CGImage? { streak(width: 6, height: 44, peakAlpha: 0.85) }
